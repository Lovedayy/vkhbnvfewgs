...
void CFilterDlg::OnBnClickedOk()
{
    ...
    if (1 == m_http.GetCheck())
    {
        filtername += _T("(tcp and ip) and (src port 80 or dst port 80) or ");
    }
    if (1 == m_dns.GetCheck())
    {
        filtername += _T("udp and dst port 53 or ");
    }

    // 如果至少有一个协议被选中，则删除最后的"or"字符串
    if (m_tcp.GetCheck() == 1 || m_udp.GetCheck() == 1 || m_arp.GetCheck() == 1 || m_icmp.GetCheck() == 1 || m_http.GetCheck() == 1 || m_dns.GetCheck() == 1)
